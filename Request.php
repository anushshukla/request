...
class Request
{
  ...
  private function __construct()
  {
    ...
    $this->_data    = $_REQUEST + $this->_files;
    if(!empty($this->body)) {
  ...
  {
    return $this->_method === strtolower(trim($method));
  }

  public function isContent($contentType, $contentTypeSecondary = NULL)
  {
    if(!$contentTypeSecondary) :
      return $this->content === $contentType;
    endif;
    return $this->contentPrimary === $contentType && $this->contentSecondary === $contentTypeSecondary;
  }

  public static function __callStatic($method = '', array $arguments = array())
  {
    $obj = new self();
    if(!method_exists($obj, $method) || $this->__get($method)) {
      throw new Exception("$method does not exists", 1);
    }
    return call_user_method_array($method, $obj, $arguments);
  }
  ...
  private function getHeader($headerName = "")
  {
    ...
    ? strtolower(trim($this->_headers[(string)$headerName]))
    : '';
  }
